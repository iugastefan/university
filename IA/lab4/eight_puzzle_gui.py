...
def draw_game_window(pozitii):
    ...
    for i in range(len(pozitii)):
        if i < 3:
            ...
            win.blit(number_list[pozitii[i]], (200 * (i - 3), 200))
        else:
            win.blit(number_list[pozitii[i]], (200 * (i - 6), 400))
...
def start_page(win, pozitii, scop):
    ...
    while start_page_boolean:
        ...
        button("Human player", 250, 400, 300, 50, dark_slate_blue, cornflower_blue, human_player, param1=pozitii,
               param2=scop)
        pygame.display.update()
